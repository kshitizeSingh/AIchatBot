/**
 * UserNavigator Unit Tests
 * 
 * Tests the user navigation component including:
 * - Tab navigator rendering and structure
 * - Stack navigator for chat section
...
jest.mock('../../screens/user/ChatScreen', () => {
...
jest.mock('../../screens/user/HistoryScreen', () => {
  const { Text } = require('react-native');
...
jest.mock('../../screens/user/ProfileScreen', () => {
  const { Text } = require('react-native');
...
describe('UserNavigator', () => {
  ...
  /**
   * Test: Should render user navigator with tab structure
   * Scenario: UserNavigator component mounts
   * Expected: Tab navigator renders with proper structure
   */
  it('should render user navigator with tab structure', () => {
   ...
   * Expected: Chat, History, and Profile tabs are visible
   */
  ...
  /**
   * Test: Should configure chat stack navigator correctly
   * Scenario: Chat tab contains stack navigator with chat screen
   * Expected: Stack navigator is properly configured
   */
  it('should configure chat stack navigator correctly', () => {
    const component = render(
      <NavigationContainer>
        <UserNavigator />
      </NavigationContainer>
   ...
   * Scenario: UserNavigator creates correct navigation structure
  ...
  /**
   * Test: Should configure header styles for chat stack
   * Scenario: Chat stack navigator has proper header styling
   * Expected: Header styles are applied to stack screens
   */
  it('should configure header styles for chat stack', () => {
    const component = render(
      <NavigationContainer>
        <UserNavigator />
      </NavigationContainer>
  ...
  it('should render without navigation provider', () => {
    ...
    const component = render(<UserNavigator />);
    expect(component).toBeDefined();
  });
  ...
  /**
   * Test: Should configure chat screen title correctly
   * Scenario: Chat stack navigator has proper screen title
   * Expected: Chat screen shows "AI FAQ Assistant" title
   */
  it('should configure chat screen title correctly', () => {
    const component = render(
      <NavigationContainer>
        <UserNavigator />
      </NavigationContainer>
  ...
  /**
   * Test: Should handle different route names correctly
   * Scenario: Tab navigator handles all defined routes
   * Expected: ChatTab, History, and Profile routes are properly configured
   */
  it('should handle different route names correctly', () => {
    const component = render(
      <NavigationContainer>
        <UserNavigator />
      </NavigationContainer>
